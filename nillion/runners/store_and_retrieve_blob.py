...
async def main():
    ...
    txt = Path('../../dataset/blob').read_text()
    print(f"Size of blob is : {len(txt)}")

    # create a bytearray from the string using UTF-8 encoding
    secret_value = bytearray(txt, "utf-8")
    ...
    print(f"The secret value len is '{len(decoded_secret_value)}'")
